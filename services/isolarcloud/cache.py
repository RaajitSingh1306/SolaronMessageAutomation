...
def _get_cache_path(month: str) -> str:
    ...
    filename = f"isolarcloud_{month}.json"
    return os.path.join(Config.ISOLARCLOUD_CACHE_DIR, filename)
...
def is_cache_fresh(month: str) -> bool:
    ...
    try:
        record = db.query(MonthlyGeneration).filter(
            MonthlyGeneration.platform == 'isolarcloud',
    ...
    try:
        ...
        for r_dict in records_data:
            rec = PlantRecord(
                ...
                source=r_dict.get("source", "isolarcloud"),
...
def clear_cache(month: Optional[str] = None) -> None:
    ...
    try:
        if month:
            ...
            logger.info("Cleared all isolarcloud cache")
        db.commit()
    except Exception as e:
