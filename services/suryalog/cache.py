...
def _get_cache_path(month: str) -> str:
    ...
    filename = f"suryalog_{month}.json"
    return os.path.join(Config.SURYALOG_CACHE_DIR, filename)
...
def is_cache_fresh(month: str) -> bool:
    ...
    try:
        record = db.query(MonthlyGeneration).filter(
            MonthlyGeneration.platform == 'suryalog',
    ...
    try:
        ...
        for r_dict in records_data:
            rec = PlantRecord(
                ...
                source=r_dict.get("source", "suryalog"),
...
def clear_cache(month: Optional[str] = None) -> None:
    ...
    try:
        if month:
            ...
            logger.info("Cleared all suryalog cache")
        db.commit()
    except Exception as e:
